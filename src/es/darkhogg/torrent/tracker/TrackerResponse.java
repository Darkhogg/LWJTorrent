...
/**
 * Represents the response to a request to a {@link Tracker}. This class is immutable.
 * <p>
...
public final class TrackerResponse {
    ...
    /**
     * Returns a human-readable message describing the reason of this request failure, if any. If the tracker didn't
     * send a failure message, this method returns <tt>""</tt>.
     * 
    ...
    /**
     * Returns a human-readable warning message returned by the tracker. If the tracker didn't returned any warning,
     * this method returns <tt>""</tt>.
     * 
    ...
    /**
     * Returns the number of seconds that the client should wait between regular announces. If the request failed, this
     * method return 0.
     * 
    ...
    /**
     * Returns the number of seconds that the client must wait between announces, regular or not. If the request failed,
    ...
    /**
     * Returns the tracker ID that must be sent in subsequent announces. If the request failed or the tracker didn't
     * send this value, this method returns <tt>null</tt>. Otherwise, it returns a byte array.
     * 
    ...
    /**
     * Returns the number of peers available that has completed the torrent download. If the request failed, this method
     * returns 0.
     * 
    ...
    /**
     * Returns the number of peers available that has not completed the torrent download. If the request failed, this
     * method returns 0.
     * 
    ...
    /**
     * Returns the list of peers returned by the tracker. If the request failed, this method returns an empty list.
     * 
    ...
    /**
     * Returns a <tt>TrackerResponse</tt> constructed using the given bencode value.
     * 
     ...
     * @throws IllegalArgumentException if <tt>value</tt> is not a valid response
     */
    public static TrackerResponse fromValue (Value<?> value) {
        ...
        try {
            ...
            if (failReasV == null) {
                ...
                long incomplete = ((IntegerValue) Bencode.getChildValue(value, "incomplete")).getValue().longValue();

                List<PeerInfo> peers = readPeers(Bencode.getChildValue(value, "peers"));

                return new TrackerResponse(
                    false, "", warning, interval, minInterval, trackerId, complete, incomplete, peers);
            } else {
        ...
        if (value instanceof StringValue) {
            ...
            for (Value<?> v : list.getValue()) {
                ...
                try {
                    ...
                    peers.add(new PeerInfo(new InetSocketAddress(addr, port), (peerId == null ? null : new PeerId(
                        peerId))));

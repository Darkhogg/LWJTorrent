...
/**
 * A class used to decode instances of {@link BitTorrentMessage} from
 * {@link ByteBuffer}s.
 * 
 * @author Daniel Escoz
 * @version 1.0
...
public final class BitTorrentMessageDecoder {
	...
	/**
	 * Decodes a handshake start (protocol name, reserved bits and info hash)
	 * from the passed <tt>buffer</tt>.
	 * 
	 * @param buffer Buffer containing at least enough bytes to read a full
	 *        handshake minus the last 20 bytes
	 * @return A <tt>HandShakeStart</tt> message representing the handshake
	 *         read
	 * @throws BufferUnderflowException if there are not enough bytes to read
	 *         the handshake start
	 */
	public static HandShakeStart decodeHandShakeStartFromBuffer (
		ByteBuffer buffer
	) {
		// Protocol name
	 ...
	 * @return A <tt>HandShakeEnd</tt> message representing the handshake
	 *         read
	 * @throws BufferUnderflowException if there are not enough bytes to read
	 *         the handshake end
	 */
	public static HandShakeEnd decodeHandShakeEndFromBuffer (
		ByteBuffer buffer
	) {
		// Peer ID
	 ...
	 * @throws BufferUnderflowException if there aren't enough bytes to read a
	 *         full message.
	 * @throws IllegalArgumentException if the message trying to be decoded is
	 *         unrecognized.
	 */
	public static BitTorrentMessage decodeMessageFromBuffer (
		ByteBuffer buffer
	) {
	...
	/**
	 * Decodes a regular message of <tt>length</tt> bytes from the given
	 * <tt>buffer</tt>.
	 * <p>
	 ...
	 * <p>
	 * Note that the <tt>length</tt> argument is the length of the bitfield,
	 * and not the length of the message payload.
	 * 
	 * @param buffer Buffer containing the message
	 * @param length Length, in bytes, of the bitfield
	 * @return Decoded message
	 */
	private static BitTorrentMessage decodeBitField (
		ByteBuffer buffer, int length
	) {
	...
	private static BitTorrentMessage decodeRequest ( ByteBuffer buffer ) {
		return new CancelMessage( buffer.getInt(),
	 ...
	 * <p>
	 * Note that the <tt>length</tt> argument is the length of the block,
	 * and not the length of the message payload.
	 * 
	 * @param buffer Buffer containing the message
	 ...
	 * @return Decoded message
	 */
	private static BitTorrentMessage decodePiece (
		ByteBuffer buffer, int length
	) {
		return new PieceMessage( buffer.getInt(), buffer.getInt(),
			buffer, length );
	}

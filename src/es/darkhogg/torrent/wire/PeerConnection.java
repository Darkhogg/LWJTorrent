...
/**
 * Represents a connection with a peer using the BitTorrent wire protocol. This class is intended as a simple
 * abstraction over a {@link java.nio.channels.SocketChannel SocketChannel}, but does not provide any associated state.
 * 
 * @author Daniel Escoz
 * @version 1.0
 */
public final class PeerConnection implements Closeable {
    ...
    /**
     * Returns a message from the input queue. If no message is available, this method will wait until one arrives.
     * <p>
    ...
    /**
     * Creates a new <tt>PeerConnection</tt> using an already connected socket channel.
     * <p>
    ...
    /**
     * Creates a new <tt>PeerConnection</tt> using a new channel connected to the given address
     * <p>
     * The connection will use <tt>ibufSize</tt> and <tt>obufSize</tt> as the size for the input and output buffers. The
     * actual size is <i>at least</i> the values given here, but this method ensures that a <i>Piece</i> message with a
     * block of size <tt>ibuf</tt>/<tt>obuf</tt> can be respectively sent/received.

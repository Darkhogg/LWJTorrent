...
/**
 * A further abstraction over {@link PeerConnection}. This class maintains the
 * state of the connection and automatically process it in a background thread,
 * firing events on its listeners when a message arrives, a message is sent, or
...
public final class PeerSession implements Closeable {
	...
	/**
	 * Returns the protocol name sent with the handshake start, or <tt>null</tt>
	...
	/**
	 * Returns the info hash received with the handshake start, or <tt>null</tt>
	...
	/**
	 * Returns the reserved bits sent with the handshake start, or <tt>null</tt>
	...
	/**
	 * Returns <tt>true</tt> if and only if a <i>HandShake Start</i> message has
	 * been sent using both the {@link #sendMessage} and {@link #process}
	 * methods.
	 * 
	...
	/**
	 * Returns <tt>true</tt> if and only if a <i>HandShake Start</i> message has
	 * been received using the {@link #process} method. Whether the message has
	 * been actually read using {@link #receiveMessage} is not relevant to the
	...
	/**
	 * Returns <tt>true</tt> if and only if a <i>HandShake End</i> message has
	 * been sent using both the {@link #sendMessage} and {@link #process}
	 * methods.
	 * 
	...
	/**
	 * Adds a message to this session internal queue so it is sent as soon as
	 * possible.
	 * 
	 * @param message
	 *            Message to be sent.
	...
	/**
	 * Adds a <i>peer listener</i> that will receive events when messages are
	 * receive or sent, or when this session is closed.
	 * 
	 * @param pl
	 *            The listener to be added
	 * @throws NullPointerException
	 *             if <tt>pl</tt> is <tt>null</tt>
	 */
	 ...
	 *            Message received
	 */
	private void fireReceiveEvent ( BitTorrentMessage msg ) {
	 ...
	 *            Message sent
	 */
	private void fireSendEvent ( BitTorrentMessage msg ) {
	 ...
	 * @version 1.0
	 */
	private final class SendThread implements Runnable {
	...
	/**
	 * A <tt>Runnable</tt> that is in charge of firing <i>message received</i>
	 * events
	 * 
	 * @author Daniel Escoz
	 * @version 1.0
	 */
	private final class ReceiveFirer implements Runnable {
		
		private final BitTorrentMessage message;
		
		public ReceiveFirer ( BitTorrentMessage message ) {
	...
	/**
	 * A <tt>Runnable</tt> that is in charge of firing <i>message sent</i>
	 * events
	 * 
	 * @author Daniel Escoz
	 * @version 1.0
	 */
	private final class SendFirer implements Runnable {
		
		private final BitTorrentMessage message;
		
		public SendFirer ( BitTorrentMessage message ) {
			this.message = message;
		}
	 ...
	 * @version 1.0
	 */
	private final class CloseFirer implements Runnable {
